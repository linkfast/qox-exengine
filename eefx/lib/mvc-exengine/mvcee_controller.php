...
class Controller {
	...
	final function loadDB($dbObj="default") {
		if (is_array($dbObj)) {
			$this->ee->errorExit('MVC-ExEngine','$this->loadDb() only accepts the name of the database configuration file (that are inside config/database folder), using EEDBM object is deprecated.');
		} else {
			...
			if ($dbObj!='default') {
				if (file_exists('config/database/' . $dbObj . '.yml')) {
					...
					$this->ee->errorExit('MVC-ExEngine','Database configuration file ('.$dbObj.') not found.');
				}
			}
			if ($DBArr['type']=='mongodb')
				$this->ee->errorExit('MVC-ExEngine','MongoDB is not supported by ExEngine Database Manager, you can use this database only with DBO Models.');
			else {
