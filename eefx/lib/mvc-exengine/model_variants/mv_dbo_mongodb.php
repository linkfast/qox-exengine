...
ExEngine / Libs / MVC-ExEngine / Model Class Variants / DBO MongoDB
		...
		if ($DeleteDefaults) {
            ...
            foreach (array_keys($vars) as $key) {
				if (isset($vars[$key])) {
					...
					//print_r($com_vars[$key]);
                    if (get_class($com_vars[$key]) == "MongoDate") {
                        unset($vars[$key]);
                    }
				}
			}
	...
	final function load($SafeMode=true, $DeleteDefaults=false) {	
		...
		if (isset($this->$ik)) {	
			...
			try {                
				...
				if ($this->INDEXKEY == '_mongo_id' || $this->INDEXKEY == 'id') {
					$find_array = array( 
						"_id" => new \MongoId($this->$ik)
					);
				} else {
					$find_array = array(
						$this->INDEXKEY => urlencode($this->$ik)
					);
				}
...
*/
	...
	function load_all($WhereArray=null,$SafeMode=true,$SortArray=null) {
		...
		foreach ($mongoResult as $key => $obj) {
            ...
            if ($this->INDEXKEY == '_mongo_id' or $this->INDEXKEY == 'id')
		        $obj[$this->INDEXKEY] = @$obj['_id']->__toString();
            else
