...
ExEngine / Libs / MVC-ExEngine / Model Class Variants / DBO SQLite

ExEngine MVC Implementation Library

 */

class eemvc_model_dbo_sqlite extends eemvc_model {
	...
	private function sql_automode() {
		...
		$this->loadDb($this->DBCONN);
		$this->db->open();
		$q = $this->db->query("PRAGMA table_info('" . $this->TABLEID . "')");
		$pkc = 0;
		while ($row = $this->db->fetchArray($q,true,EEDBM_ASSOC)) {
			$fieldName = $row["name"];
			$this->$fieldName = null;
	...
	final function load($SafeMode=true) {
		...
		if (isset($this->$ik)) {
			...
			$data = $this->db->fetchArray($q,$SafeMode,EEDBM_ASSOC);
