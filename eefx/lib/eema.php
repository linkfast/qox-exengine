...
class eema {
	...
	/**
	 * @param string $applicationShortName	Shortname should be used in the EEMA client interface.
	 * @param string $applicationLongName	Longname should be used in the EEMA client interface to get a brief description of the app.
	 * @param bool   $fallBackInLegacy
	 */
...
//		$this->ma = new eema('eema-lib','EE Message Agent Lib');
	...
	/**
	 * @param      $appKey
	 * @param bool $inReverseOrder
	 * @param bool $nl2br
	 * @param bool $additionalDataParse
	 * @param null|array $filters
	 *
	 * @return array
	 */
	public static function getMessages($appKey,$inReverseOrder=false,$nl2br=false, $additionalDataParse=false, $filters=null) {
		if (isset($_SESSION['exengine-eema-messages'][$appKey]) && is_array($_SESSION['exengine-eema-messages'][$appKey])) {
			...
			if ($nl2br || $additionalDataParse) {
				foreach ($data as $key => $element) {
					...
					if (is_array($filters)) {
						//$m = new eema('eema-filters','EE Message Agent Filters');
						//$m->t('Filters',$filters);
						foreach($filters as $filterkey => $filter) {
							//$m->t('Filter Check ' . $filterkey . ' : ' . $filter);
							if ($filter=="false") {
								//$m->t('Level ' . $data[$key]['level'] );
								if ($data[$key]['level'] == $filterkey) {
									//$m->t('Filter ' . $filterkey . ' unset');
									unset($data[$key]);
								}
							}
						}
