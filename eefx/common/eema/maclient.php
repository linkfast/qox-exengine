						...
						<ul class="nav navbar-nav navbar-right">
							...
							<li class="dropdown dropdown-keepopen">
								...
								<ul class="dropdown-menu" role="menu">
									...
									<li><a href="#" onclick="eema_toggleFilter('info'); return false;">INFO   		<span id="badge_info" class="badge" id="autorefresh-badge">ON</span></a></li>
									<li><a href="#" onclick="eema_toggleFilter('warning'); return false;">WARNING 	<span id="badge_warning" class="badge" id="autorefresh-badge">ON</span></a></li>
									<li><a href="#" onclick="eema_toggleFilter('error'); return false;">ERROR 		<span id="badge_error" class="badge" id="autorefresh-badge">ON</span></a></li>
									<li><a href="#" onclick="eema_toggleFilter('fatal'); return false;">FATAL 		<span id="badge_fatal" class="badge" id="autorefresh-badge">ON</span></a></li>
								</ul>
							</li>
						</ul>
					</div><!-- /.navbar-collapse -->
...
var module_init = function() {
	...
	setTimeout(function() {
		...
		/* dropdown filters disable onclick closing. */
...
var DDOpened=false;
var eema_toggleDropdown = function() {
	if (!DDOpened)
	{
		$('.dropdown-keepopen').addClass('open');
		DDOpened=true;

		$("#filters_toggle_btn").attr('title','Click here to close and refresh messages.');
		$("#filters_toggle_btn").tooltip();
		$("#filters_toggle_btn").tooltip('enable');
		$("#filters_toggle_btn").tooltip('show');
	}
	else
	{
		$('.dropdown-keepopen').removeClass('open');
		$("#filters_toggle_btn").removeAttr('title');
		$("#filters_toggle_btn").tooltip('disable');
		DDOpened=false;
		if (eema_selApp != null)
...
var eema_filters = new Object();
eema_filters['trace'] = true; eema_filters['debug'] = true; eema_filters['info'] = true; eema_filters['warning'] = true; eema_filters['error'] = true; eema_filters['fatal'] = true;

var eema_toggleFilter = function(filterName) {
	if (eema_filters[filterName]) {
		eema_filters[filterName] = false;
		$("#badge_"+filterName).html("OFF");
	} else {
		eema_filters[filterName] = true;
		$("#badge_"+filterName).html("ON");
	}
};
...
var eema_getNewMessages = function() {
	if (eema_selApp != null) {
		...
		$.ajax({
			type: 'POST',
			url: eema_server,
			data: { 'cmd' : 'getMessages', 'appKey' : eema_selApp, 'filters' : eema_filters },
			dataType: 'json'
		}).fail(function() {
...
var eema_loadApp = function(eemaApp,appTitle) {
	...
	$.ajax({
		type: 'POST',
		url: eema_server,
		data: { 'cmd' : 'getMessages', 'appKey' : eemaApp, 'filters' : eema_filters },
		dataType: 'json'
	}).fail(function() {
		alert("error!");
		eema_notBusy();
